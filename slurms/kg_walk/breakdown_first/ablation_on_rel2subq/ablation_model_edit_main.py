...
from kg_utils import get_relation, get_fact_form_kg, fit_subject_on_kg, get_relation2
...
rel2subq = {
    '{} is a citizen of': 'Who is a citizen of {}?',
...
def main():
    ...
    ablation_code = args.ablation_code
...
def evaluate_on_dataset_kg_walk_breakdown_first(dataset, task_prompt, sc_facts, model, gptj_tokenizer, device, rels,
                                                ...
                                                rand_list, print_prompt, breakdown_prompt, S=0, ablation_code=0,
                                                T=200):
    cor = 0
    tot = 0
    ...
    for d in tqdm(dataset[S:T]):
        ...
        for q_id, q in enumerate(d["questions"]):
            ...
            for i in range(len(breakdown_rels)):
                if ablation_code == 0:
                ...
                elif ablation_code == 1:
                    # replace "{} was performed by" by "performer" when getting subq.
                    relation = breakdown_rels[i]
                    rel = get_relation(relation, rels, rel_emb, contriever, tokenizer)
                    subquestion = fetch_rel_subj2subq(subject, relation, relation2subq_prompt, sc_end_block, model=model,
                                                      gptj_tokenizer=gptj_tokenizer, device=device)
                elif ablation_code == 2:
                    # format the fetched rel into subq directly
                    relation = breakdown_rels[i]
                    rel = get_relation(relation, rels, rel_emb, contriever, tokenizer)
                    
                    subquestion = rel2subq[rel].format(subject)
                    
                elif ablation_code == 3:
                    # enabled get_relation2
                    relation = breakdown_rels[i]
                    rel = get_relation2(relation, rels, rel_emb, contriever, tokenizer)
                    if rel is None:
                        key_word_rel2subq = relation
                    else:
                        key_word_rel2subq = rel
                    subquestion = fetch_rel_subj2subq(subject, key_word_rel2subq, relation2subq_prompt, sc_end_block, model=model,
                                                      gptj_tokenizer=gptj_tokenizer, device=device)
                elif ablation_code == 4:
                    # use llm to prompt subq if rel is none, or use rel.format otherwise
                    relation = breakdown_rels[i]
                    rel = get_relation2(relation, rels, rel_emb, contriever, tokenizer)
                    if rel is None:
                    ...
                    raise ValueError(f"Ablation code {ablation_code} not implemented.")
