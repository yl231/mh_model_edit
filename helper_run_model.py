...
def fetch_rel_subj2subq(subject, rel, relation2subq_prompt, sc_end_block, model, gptj_tokenizer, device):
    prompt = relation2subq_prompt + f"Given this relation: \"{rel}\" and this subject: \"{subject}\",\nThe corresponding question is"
    output = call_model(prompt, sc_end_block, temperature=0.2, generate_length=20, model=model, gptj_tokenizer=gptj_tokenizer, device=device)
